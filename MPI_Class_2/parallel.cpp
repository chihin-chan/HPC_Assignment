...
void PostProc(double* s, int nx, int ny, int* coords, double dx, double dy){
		string cordx = to_string(coords[0]);
		string cordy = to_string(coords[1]);
		ofstream sOut("streamfunction"+cordx+cordy+".txt", ios::out | ios::trunc);
		sOut.precision(5);
		sOut << setw(15) << "x"
	    	 << setw(15) << "y"
	     	 << setw(15) << "psi" << endl;
		for(int i=1; i<nx-1; i++){
			for(int j=1; j<ny-1; j++){
				sOut << setw(15) <<  (coords[1]*(nx-2)+i-1)*dx
			    	 << setw(15) <<  (coords[0]*(ny-2)+j-1)*dy
...
void MapPoiSolve(double* s, double* v, double dx, double dy, int nx, int ny,
	...
	for(int i = 0; i < nx-x_off; i++){
		for(int j = 0; j < ny-y_off; j++){
			s[(i+x_start) + nx*(j+y_start)] = rhs[i+j*(nx-x_off)];
		}
	}
...
while(T<T_final){
	...
	// matPrintRank(s, loc_nx, loc_ny, rank, 1);
	/////////////////////////////////////////////////////////////
	// Packing/Sending Receiving/Unpacking from/to right
	/////////////////////////////////////////////////////////////
	if(nghbrs[RIGHT] != -2){
	...
	matPrintRank(s, loc_nx, loc_ny, rank, 0);

	if(rank == 0){
		PostProc(s, loc_nx, loc_ny, coords, dx, dy);
	}
	// Left Columns except corners
	else if(coords[1] == 0 && rank != 0 && coords[0] != Px-1){
		MapPoiSolve(s, v, dx, dy, loc_nx, loc_ny, 3, 2, 2, 1, 1, 0, rank);
